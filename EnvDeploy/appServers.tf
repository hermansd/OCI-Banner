...
resource "oci_core_instance" "appServer" {
  ...
  availability_domain = var.app_multiple_ads ? (contains(range(1,var.app_host_count , 2), count.index) ? var.target_ad2 : var.target_ad1) : var.target_ad1
  ...
  source_details {
    source_type = "image"
    source_id   = var.app_change_ssh ? var.app_sshkey : var.sshkey
  ...
  metadata = {
    ssh_authorized_keys = var.app_sshkey
  }
